...
public sealed class SeedVcWorkflow : ISeedVcWorkflow
{
    ...
    private readonly SeedVcRemoteTrainService _remoteTrain = new();
    ...
    public async Task<SeedVcRemoteTrainResult> RemoteTrainAsync(SeedVcRemoteTrainRequest request, WorkflowContext context, CancellationToken cancellationToken)
    {
        EnsureStart(context);
        try
        {
            var result = await _remoteTrain.TrainAsync(request, context.Progress, cancellationToken).ConfigureAwait(false);
            context.StateMachine.TryComplete();
            return result;
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { context.StateMachine.TryCancel(); throw; }
