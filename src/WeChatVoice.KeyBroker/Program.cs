...
if (args.Length == 4
    ...
    && string.Equals(args[2], "--worker-directory", StringComparison.Ordinal))
{
    try
    {
        return await BrokerPipeServer.RunSelfTestAsync(
            args[1],
...
try
{
    return await BrokerPipeServer.RunAsync(
        ...
        allowExperimentalProfile: false).ConfigureAwait(false);
}
catch (Exception exception) when (exception is ArgumentException or IOException or UnauthorizedAccessException or OperationCanceledException)
{
    return 2;
