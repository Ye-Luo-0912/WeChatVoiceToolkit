...
namespace WeChatVoice.Infrastructure.Trust;

public sealed record WorkerBundleVerificationResult(bool Verified, string? NonSensitiveReason)
{
    public static WorkerBundleVerificationResult Ok() => new(true, null);
    public static WorkerBundleVerificationResult Deny(string reason) => new(false, reason);
}

/// <summary>
/// Authoritative read-only verification of the SQLCipher Worker load closure.
/// Both the normal workflow preflight and the elevated Broker self-test use
/// this implementation.
/// </summary>
public static class WorkerBundleTrustEvaluator
{
    public static async Task<WorkerBundleVerificationResult> VerifyAsync(string directory, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return WorkerBundleVerificationResult.Deny("worker-bundle-unavailable");
            }
            ...
            {
                return WorkerBundleVerificationResult.Deny("worker-hash-mismatch");
            }
                     ...
                     })
            {
                ...
                {
                    return WorkerBundleVerificationResult.Deny("worker-bundle-entry-missing");
                }
                ...
                {
                    return WorkerBundleVerificationResult.Deny("worker-bundle-entry-invalid");
                }
                ...
                {
                    return WorkerBundleVerificationResult.Deny("worker-bundle-sidecar-mismatch");
                }
            }

            if (!json.TryGetProperty("files", out var files) || files.ValueKind != JsonValueKind.Array)
            {
                return WorkerBundleVerificationResult.Deny("worker-bundle-closure-missing");
            }

            return await VerifyCompleteClosureAsync(directory, root, files, cancellationToken).ConfigureAwait(false)
                ? WorkerBundleVerificationResult.Ok()
                : WorkerBundleVerificationResult.Deny("worker-bundle-closure-mismatch");
        }
        catch (JsonException)
        {
            return WorkerBundleVerificationResult.Deny("worker-bundle-invalid");
        }
        catch (IOException)
        {
            return WorkerBundleVerificationResult.Deny("worker-bundle-unavailable");
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in files.EnumerateArray())
        {
            ...
            if (!expected.Add(normalized)) return false;
            var path = Path.GetFullPath(Path.Combine(directory, relative));
            if (!path.StartsWith(root, StringComparison.OrdinalIgnoreCase) || !IsSafeFile(path, root)) return false;
            var info = new FileInfo(path);
            if (info.Length != expectedLength
                || !string.Equals(await FileHashing.ComputeSha256Async(path, cancellationToken).ConfigureAwait(false), expectedHash, StringComparison.OrdinalIgnoreCase)) return false;
        }
        ...
        var actual = Directory.EnumerateFiles(directory, "*", SearchOption.AllDirectories)
            ...
            .Select(path =>
            {
                if ((File.GetAttributes(path) & FileAttributes.ReparsePoint) != 0) throw new InvalidDataException("The Worker bundle contains a reparse-point file.");
